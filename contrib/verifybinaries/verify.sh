...
###   bitcoinarraycore.org and bitcoinarray.org and compares them.
...
WORKINGDIR="/tmp/bitcoinarray_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinarraycore.org"
HOST2="https://bitcoinarray.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinarray-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinarray-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinarray.org failed to provide signature file, but bitcoinarraycore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinarray.org and bitcoinarraycore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinArray Core binary release signing key installed?"
   fi
