...
The RPM spec file provided here is for BitcoinArray-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinarray.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitcoinArray git repository](https://github.com/bitcoinarray/bitcoinarray) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinarray.conf` file.
...
in the bitcoinarray git repository. At present only an SVG version of the BitcoinArray
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinarray
...
needed when 0.12.1 is released, a proper fix is already in the BitcoinArray git
master branch.
...
idea to build BitcoinArray Core against a static version of that library compiled
...
The `bitcoinarray.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinarray-server` package. At this point, most RPM based
...
The `bitcoinarray-server` package should have SELinux support. How to properly do
...
BitcoinArray-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinarray/). He has been
packaging BitcoinArray for Fedora at least since 2012.
...
4. I renamed the `bitcoinarray` package that contains the Qt GUI to `bitcoinarray-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinarray-xt` or `bitcoinarray-classic`. I wanted to make sure the general population
